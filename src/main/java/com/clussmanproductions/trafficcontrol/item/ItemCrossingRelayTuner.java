...
public class ItemCrossingRelayTuner extends Item {
		...
		if (te instanceof TrafficLightControlBoxTileEntity)
		{
			...
			if (state.getBlock() instanceof BlockTrafficSensorLeft || state.getBlock() instanceof BlockTrafficSensorStraight || state.getBlock() instanceof BlockTrafficSensorRight)
			{
				if (tlBox.addOrRemoveSensor(pos))
				{
					...
					world.playSound(null, pos, ModSounds.tunerLinkAdd, SoundCategory.PLAYER, 0.3F, 1F);
				}
				else
				{
					player.sendMessage(new TextComponentString("Unpaired sensor from Traffic Light Control Box"));
					world.playSound(null, pos, ModSounds.tunerLinkRemove, SoundCategory.PLAYER, 0.3F, 1F);
				}
			}
		}
	}
	...
	private boolean performPairCheck(EntityPlayer player, World world, TileEntity te)
	{
		...
		if (nbt == null || !nbt.hasKey("pairingpos"))
		{
			...
			world.playSound(null, pos, ModSounds.tunerLinkStart, SoundCategory.PLAYER, 0.3F, 1F);
		...
		else
		{
			...
			if (te != null && (te instanceof RelayTileEntity || te instanceof TrafficLightControlBoxTileEntity))
			{
				...
				world.playSound(null, pos, ModSounds.tunerLinkStop, SoundCategory.PLAYER, 0.3F, 1F);
			...
			else if (te != null)
			{
				...
				if (teAtPairingPos == null || (!(teAtPairingPos instanceof RelayTileEntity) && !(teAtPairingPos instanceof TrafficLightControlBoxTileEntity)))
				{
					...
					world.playSound(null, pos, ModSounds.tunerLinkError, SoundCategory.PLAYER, 0.3F, 1F);
